...
from jinja2 import Template, Environment, FileSystemLoader, exceptions, pass_context
                    ...
                    for subproperty in PROPERTY_FIELDS[p['type']]:
                        if type(subproperty) is tuple:
                            subproperties.append(
                                '{}: (( .properties.{}.{}.{}.{} ))'.format(subproperty[0], input['name'],
                                                                           option['name'], p['name'], subproperty[1]))
                        else:
                            subproperties.append(
                                '{}: (( .properties.{}.{}.{}.{} ))'.format(subproperty, input['name'], option['name'],
                                                                           p['name'], subproperty))
                    properties += ', '.join(subproperties) + ' }\r\n'
                else:
                    properties += p['name'] + ': (( .properties.' + input['name'] + '.' + option['name'] + '.' + p[
                        'name'] + '.value ))\r\n'
